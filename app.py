...
def get_oi_support_resistance(options_data):
    """
    Analyzes options chain to find the Strike Price with the highest Call OI (Resistance)
    and highest Put OI (Support), and calculates PCR.
    """
    chain = options_data.get('data', {}).get('optionsChain', [])
...
def create_comparison_chart(data_1, data_2, symbol1='Symbol 1', symbol2='Symbol 2', resolution='5 mins'):
    df1 = pd.DataFrame(data_1.get('candles', []), columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
    timestamps_1 = []
    closing_prices_1 = []
...
# --- PLOTLY COMPARISON CHART FUNCTION (Updated with OI Levels) ---
def aacreate_comparison_chart_plotly(data_1, data_2, symbol1='Symbol 1', symbol2='Symbol 2', resolution='5 mins', oi_levels=None):
    """
    Generates an interactive price comparison chart using Plotly.
    Now supports overlaying Options Support/Resistance levels.
    """
    ...
    # --- NEW: Add Options Trend Lines ---
    # Only add lines if Symbol 1 price is somewhat close to strike price (e.g., within 50%)
    # This prevents scaling issues if plotting a stock (Price 300) vs Nifty Options (Strike 25000)
    if oi_levels and closing_prices_1:
        last_price = closing_prices_1[-1]

        # Add Resistance Line (Red)
        if oi_levels['resistance']:
            # Safety check: Is strike within reasonable range of price?
            if abs(last_price - oi_levels['resistance']) / last_price < 0.5:
                fig.add_hline(
                    y=oi_levels['resistance'],
                    line_dash="dot",
                    line_color="#FF5255",
                    annotation_text=f"Max Call OI (Res): {oi_levels['resistance']}",
                    annotation_position="top right",
                    annotation_font_color="#FF5255"
                )

        # Add Support Line (Green)
        if oi_levels['support']:
            # Safety check
            if abs(last_price - oi_levels['support']) / last_price < 0.5:
                fig.add_hline(
                    y=oi_levels['support'],
                    line_dash="dot",
                    line_color="#00C853",
                    annotation_text=f"Max Put OI (Sup): {oi_levels['support']}",
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            showgrid=True,
            gridcolor='#333',
            title=dict(text=f'Price ({symbol1})', font=dict(color='#00C853')),
    ...
    if timestamps_2:
        fig.update_yaxes(
            title=dict(text=f'Price ({symbol2})', font=dict(color='#FF5255')),
...
def bbcreate_comparison_chart_plotly(data_1, data_2, symbol1='Symbol 1', symbol2='Symbol 2', resolution='5 mins', oi_levels=None):
    """
    Generates an interactive price comparison chart using Plotly.
    Includes:
    1. Symbol 1 vs Symbol 2 Price Comparison
    2. EMA 9 and EMA 21 overlays for Symbol 1
    3. Options Support/Resistance levels (Horizontal Lines)
    """
    # --- 1. Extract Data for Symbol 1 ---
    candles_1 = data_1.get('candles', [])
    timestamps_1 = [datetime.datetime.fromtimestamp(c[0]) for c in candles_1]
    closing_prices_1 = [c[4] for c in candles_1]

    # --- 2. Extract Data for Symbol 2 ---
    candles_2 = data_2.get('candles', [])
    timestamps_2 = [datetime.datetime.fromtimestamp(c[0]) for c in candles_2]
    closing_prices_2 = [c[4] for c in candles_2]
    ...
    # --- 3. Calculate EMAs for Symbol 1 ---
    ema_9 = []
    ema_21 = []

    if closing_prices_1:
        # Create a Pandas Series to easily calculate EWM (Exponential Weighted Functions)
        price_series = pd.Series(closing_prices_1)
    ...
    # Trace 1: Symbol 1 Price (Primary Y-axis) - Main Line
    if timestamps_1:
        fig.add_trace(
            go.Scatter(x=timestamps_1, y=closing_prices_1, name=f'{symbol1} Price',
                       line=dict(color='#00C853', width=2)),
            secondary_y=False,
        )

        # --- NEW: Add EMA 9 Trace (Cyan) ---
        fig.add_trace(
            go.Scatter(x=timestamps_1, y=ema_9, name=f'{symbol1} EMA 9',
                       line=dict(color='#00E5FF', width=1.5), opacity=0.8),
            secondary_y=False,
        )

        # --- NEW: Add EMA 21 Trace (Yellow) ---
        fig.add_trace(
            go.Scatter(x=timestamps_1, y=ema_21, name=f'{symbol1} EMA 21',
                       line=dict(color='#FFD600', width=1.5), opacity=0.8),
            secondary_y=False,
        )

    # Trace 2: Symbol 2 Price (Secondary Y-axis) - Comparison Line
    if timestamps_2:
        fig.add_trace(
            go.Scatter(x=timestamps_2, y=closing_prices_2, name=f'{symbol2} Price',
                       line=dict(color='#FF5255', width=2, dash='dash')),
            secondary_y=True,
        )

    # --- 5. Add Options Trend Lines (Support/Resistance) ---
    if oi_levels and closing_prices_1:
        last_price = closing_prices_1[-1]

        # Add Resistance Line (Red)
        if oi_levels['resistance']:
            # Safety check: Prevent scaling issues if strike is wildly different from price
            if abs(last_price - oi_levels['resistance']) / last_price < 0.5:
                fig.add_hline(
                    y=oi_levels['resistance'],
                    line_dash="dot",
    ...
    # Configure Secondary Axis Labels
    if timestamps_2:
        fig.update_yaxes(
            title=dict(text=f'Price ({symbol2})', font=dict(color='#FF5255')),
            tickfont=dict(color='#FF5255'),
    ...
    # Hide secondary axis if no data for Symbol 2
    if not timestamps_2:
        fig.update_yaxes(secondary_y=True, visible=False)
...
def create_comparison_chart_plotly(data_1, data_2, symbol1='Symbol 1', symbol2='Symbol 2', resolution='5 mins', oi_levels=None):
    """
    Generates an interactive price comparison chart using Plotly.
    Includes:
    1. Symbol 1 Candlestick Chart
    2. Symbol 2 Line Chart (Comparison)
    3. EMA 9 and EMA 21 overlays for Symbol 1
    4. Options Support/Resistance levels (Horizontal Lines)
    """
    # --- 1. Extract Data for Symbol 1 (Candlestick Data) ---
    candles_1 = data_1.get('candles', [])
    timestamps_1 = []
    opens_1 = []
    highs_1 = []
    ...
    if candles_1:
        # Unpack the candle list: [timestamp, open, high, low, close, volume]
        timestamps_1 = [datetime.datetime.fromtimestamp(c[0]) for c in candles_1]
        opens_1 = [c[1] for c in candles_1]
        highs_1 = [c[2] for c in candles_1]
        lows_1 = [c[3] for c in candles_1]
        closes_1 = [c[4] for c in candles_1]

    # --- 2. Extract Data for Symbol 2 (Line Data) ---
    candles_2 = data_2.get('candles', [])
    timestamps_2 = [datetime.datetime.fromtimestamp(c[0]) for c in candles_2]
    closing_prices_2 = [c[4] for c in candles_2]
    ...
    # --- 3. Calculate EMAs for Symbol 1 (Based on Close) ---
    ema_9 = []
    ema_21 = []

    if closes_1:
        price_series = pd.Series(closes_1)
        ema_9 = price_series.ewm(span=9, adjust=False).mean()
        ema_21 = price_series.ewm(span=21, adjust=False).mean()
    ...
    # Trace 1: Symbol 1 CANDLESTICK (Primary Y-axis)
    if timestamps_1:
        fig.add_trace(
            go.Candlestick(
                x=timestamps_1,
                ...
                increasing_line_color='#00C853', # Green
                decreasing_line_color='#FF5255'  # Red
            ),
            secondary_y=False,
        )
    ...
    if timestamps_2:
        fig.add_trace(
            go.Scatter(x=timestamps_2, y=closing_prices_2, name=f'{symbol2} Price',
                       line=dict(color='#B0BEC5', width=2, dash='dash')), # Changed to Grey-ish for better contrast vs candles
            secondary_y=True,
        )

    # --- 5. Add Options Trend Lines (Support/Resistance) ---
    if oi_levels and closes_1:
    ...
    fig.update_layout(
        ...
        xaxis_title=f'Time ({resolution} Intervals)',
        xaxis=dict(showgrid=True, gridcolor='#333', zeroline=False),
        # IMPORTANT: Remove the default range slider for cleaner UI
    ...
    if timestamps_2:
        fig.update_yaxes(
            title=dict(text=f'Price ({symbol2})', font=dict(color='#B0BEC5')),
...
def get_chart_dpi(resolution):
    ...
    if 'day' in resolution: return 150
    elif 'hour' in resolution: return 120
    elif 'min' in resolution: return 100
    return 100
...
def get_refresh_interval_seconds(resolution_str):
    try:
        ...
        if len(parts) == 2:
            num = int(parts[0])
            unit = parts[1].lower()
            if 'min' in unit: interval_s = num * 60
            elif 'hour' in unit: interval_s = num * 3600
            elif 'day' in unit: interval_s = num * 86400
            else: interval_s = 5
...
def retres_value(value):
    cases = {"1 min": "1", "3 mins": "3", "5 mins": "5", "15 mins": "15", "30 mins": "30", "1 hour": "60", "2 hours": "120", "4 hours": "30", "1 day": "1D"}
...
def home():
    ...
    cross_alert = None
    oi_levels = {'resistance': None, 'support': None, 'pcr': 0}

    # Get metadata from the global OPTIONS_DATA
    options_data = OPTIONS_DATA
    expiry_data = options_data.get('data', {}).get('expiryData', [])
    default_expiry = expiry_data[0]['expiry'] if expiry_data else 'N/A'
    nearest_tuesday = get_upcoming_tuesday().strftime('%d-%m-%Y')

    options_chain = options_data.get('data', {}).get('optionsChain', [])
    index_record = next((d for d in options_chain if d.get('strike_price', -1) == -1 and 'description' in d), {})
    default_index = 'NSE:NIFTY50-INDEX'
    ...
    first_option_ltp = index_record.get('ltp', 'N/A')
    first_option_ltpch = index_record.get('ltpch', 'N/A')
    ...
    if request.method == 'POST':
        ...
        input_values['redirect_uri'] = request.form.get('redirect_uri', 'https://fy.autotest.com')
        ...
        # Placeholder for real API call (Currently using Dummy Data)
        data1 = DUMMY_DATA_1
        data2 = DUMMY_DATA_2
        # options_data = OPTIONS_DATA (Already set)
        ...
        # 1. Calculate Options Levels & PCR
        oi_levels = get_oi_support_resistance(options_data)

        plot_data = create_comparison_chart(data1, data2, input_values['symbol1'], input_values['symbol2'], input_values['resolution'])

        # 2. Pass levels to Plotly
        plotly_plot_data = create_comparison_chart_plotly(
            data1, data2, input_values['symbol1'], input_values['symbol2'],
            input_values['resolution'], oi_levels=oi_levels
        )
        ...
        # Calculate levels for initial load
        oi_levels = get_oi_support_resistance(options_data)

        plot_data = create_comparison_chart(data1, data2)
        plotly_plot_data = create_comparison_chart_plotly(data1, data2, oi_levels=oi_levels)
        oi_plot_data = create_oi_bar_chart(options_data, input_values['index_name'], input_values['selected_expiry'])
        ...
        <html lang="en">
          <head>
            ...
            <style>
              ...
              .header { text-align: center; margin-bottom: 20px; color: #ffffff; }
              .dashboard-container { display: flex; flex-direction: column; gap: 20px; max-width: 1400px; margin: 0 auto; }
              .controls { background-color: #1e1e1e; padding: 20px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.5); }
              .form-row { display: flex; flex-wrap: wrap; gap: 20px; align-items: flex-end; width: 100%; margin-top: 15px; }
              .input-group { display: flex; flex-direction: column; min-width: 150px; flex-grow: 1; }
              .input-group.lg { flex-grow: 2; min-width: 250px; }
              label { margin-bottom: 5px; color: #90CAF9; font-weight: bold; }
              input[type="text"], input[type="number"], select { padding: 10px; border: 1px solid #333; border-radius: 5px; background-color: #2c2c2c; color: #e0e0e0; min-height: 40px; }
              button { padding: 10px 20px; background-color: #00C853; color: black; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; transition: background-color 0.3s; min-height: 40px; flex-grow: 0.5; }
              button:hover { background-color: #009624; }
              .chart-container { background-color: #1e1e1e; padding: 20px; border-radius: 15px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.5); text-align: center; }
              img { max-width: 100%; height: auto; display: block; margin: 0 auto; }
              .alert-box { padding: 15px; margin-bottom: 20px; border-radius: 8px; font-weight: bold; text-align: center; animation: pulse 1s infinite alternate; }
              .alert-up { background-color: #00C853; color: black; }
              .alert-down { background-color: #FF5255; color: black; }
              .data-snapshot, .market-data-snapshot { background-color: #1e1e1e; padding: 15px 20px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.5); text-align: left; }
              .snapshot-row { display: flex; flex-wrap: wrap; gap: 40px; margin-top: 10px; }
            </style>
          </head>
          <body>
            <div class="header"><h1>Dual Price Series Chart Dashboard 📈</h1><p>Parameters for API Configuration and Data Request</p></div>
            <div class="dashboard-container">
                <div class="controls">
                    <form method="POST" id="dataForm">
                        <h2>API Configuration</h2>
                        <div class="form-row">
                            <div class="input-group lg"><label for="client_id">Client ID:</label><input type="text" id="client_id" name="client_id" value="{{ input_values['client_id'] }}" required></div>
                            <div class="input-group lg"><label for="secret_key">Secret Key:</label><input type="text" id="secret_key" name="secret_key" value="{{ input_values['secret_key'] }}" required></div>
                            <div class="input-group lg"><label for="redirect_uri">Redirect URI:</label><input type="text" id="redirect_uri" name="redirect_uri" value="{{ input_values['redirect_uri'] }}" required></div>
                        </div>
                        <h2>Price Comparison Data Request</h2>
                        <div class="form-row">
                            <div class="input-group lg"><label for="token">Access Token:</label><input type="text" id="token" name="token" value="{{ input_values['token'] }}" required></div>
                            <div class="input-group"><label for="symbol1">Symbol 1:</label><input type="text" id="symbol1" name="symbol1" value="{{ input_values['symbol1'] }}" required></div>
                        ...
                        <div class="form-row">
                            ...
                            <div class="input-group"><label for="resolution">Resolution (Chart Bars):</label>
                             ...
                             <div class="input-group"><label for="refresh_interval">Refresh Interval (Secs):</label><input type="number" id="refresh_interval" name="refresh_interval" value="{{ input_values['refresh_interval'] }}" min="5" required></div>
                        </div>
                        <h2>Options Data Request</h2>
                        <div class="form-row">
                             <div class="input-group"><label for="index_name">Index Name:</label><input type="text" id="index_name" name="index_name" value="{{ input_values['index_name'] }}" required></div>
                             <div class="input-group"><label for="selected_expiry">Expiry Date:</label>
                                <select id="selected_expiry" name="selected_expiry">
                                    {% for exp in expiry_data %}
                                        <option value="{{ exp['expiry'] }}" {% if input_values['selected_expiry'] == exp['expiry'] %}selected{% endif %}>{{ exp['date'] }}</option>
                                    {% endfor %}
                                </select>
                            </div>
                ...
                <div class="market-data-snapshot">
                    ...
                    <div class="snapshot-row">
                        <p><strong>Put-Call Ratio (PCR):</strong> <span style="font-size: 1.2em; color: {% if pcr_value|float > 1 %}#00C853{% elif pcr_value|float < 0.8 %}#FF5255{% else %}#FFD700{% endif %};">{{ pcr_value }}</span></p>
                ...
                {% if cross_alert == 'Cross Up' %}<div class="alert-box alert-up">⬆️ CROSS UP ALERT! {{ input_values['symbol1'] }} has crossed **ABOVE** {{ input_values['symbol2'] }}! 🚀</div>{% endif %}
                {% if cross_alert == 'Cross Down' %}<div class="alert-box alert-down">⬇️ CROSS DOWN ALERT! {{ input_values['symbol1'] }} has crossed **BELOW** {{ input_values['symbol2'] }}! 📉</div>{% endif %}

                {% if plotly_plot_data %}
                <div class="chart-container">
                    <h2>Interactive Price Comparison Chart (Plotly)</h2>
                    <p>Last Updated: {{ now }}. Refreshing every {{ input_values['refresh_interval'] }} seconds.</p>
                    <div id="plotly-chart">{{ plotly_plot_data | safe }}</div>
                </div>
                {% endif %}
                ...
                <div class="chart-container" style="color: #FF5255;"><p>Error: Could not generate chart. Please check the inputs.</p></div>
                {% else %}
                <div class="chart-container"><p>Enter parameters above and click 'Submit & Plot' to view the comparison chart.</p></div>
                {% endif %}
            </div>
            <script>
                ...
                if (refreshIntervalMs > 0) {
                    function autoRefresh() { dataForm.submit(); }
                    setTimeout(function() { setInterval(autoRefresh, refreshIntervalMs); }, refreshIntervalMs);
                }
            </script>
          </body>
        </html>
    ...
    return render_template_string(
        ...
        first_option_ltp=first_option_ltp,
        first_option_ltpch=first_option_ltpch,
        pcr_value=oi_levels['pcr'],
